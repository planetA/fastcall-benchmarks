...
static const unsigned long IOCTL_NOOP = fce_ioctl(0);
static const unsigned long IOCTL_STACK = fce_ioctl(1);
static const unsigned long IOCTL_PRIV = fce_ioctl(2);
static constexpr unsigned long ioctl_array(unsigned char cmd) {
  return _IOR(TYPE, cmd, struct array_args);
}
static const unsigned long IOCTL_ARRAY = ioctl_array(3);
static const unsigned long IOCTL_NT = ioctl_array(4);
